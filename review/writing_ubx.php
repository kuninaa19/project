...
$number= $_GET['id'];
    ...
    $board_name = "unboxing"; //이것은 게시판들의 중복을 막기위한 값입니다. (게시판 이름)
...
<head>
        ...
        <link href="../css/style.css?eear" rel="stylesheet" type="text/css"/>
</head>
<body>
<br>
<div class="container">
...
<?php
    ...
    if($sup == TRUE){
        ...
        if(isset($row1['id']) && $checking == $row1['id']){
         echo "<a class='btn btn-default pull-right' id= 'btn_margin' onclick='return confirm('정말로 삭제하시겠습니까?')' href='./delete_unboxing.php?id=$number'>삭제</a>";
...
<script>
  ...
  // 댓글 전부 업로드
	function getAllList(){
		...
		$.getJSON("reply_list_u.php?board=<?=$number?>", function(data){
			console.log(data);
			$(data).each(function(){
        count+=1;
